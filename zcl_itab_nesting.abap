...
CLASS zcl_itab_nesting IMPLEMENTATION.
  METHOD perform_nesting.
    ...
    nested_data = VALUE #(
        FOR ls_artist IN artists
        ( artist_id   = ls_artist-artist_id
          artist_name = ls_artist-artist_name
          albums      = VALUE #(
              FOR ls_album IN albums WHERE ( artist_id = ls_artist-artist_id )
              ( album_id   = ls_album-album_id
                album_name = ls_album-album_name
                songs      = VALUE #( FOR ls_song IN songs WHERE ( artist_id = ls_album-artist_id AND album_id = ls_album-album_id )
